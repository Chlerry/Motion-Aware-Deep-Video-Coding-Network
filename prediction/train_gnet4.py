...
def model(images, decoded, b, bm, ratio):
    ...
    # =================================================
    y0b = Conv2D(8, kernel_size=(5, 5), padding = "SAME", strides = 1, activation='relu')(y)
    y0b = Model(inputs = input1, outputs = y0b)

    y0c = Conv2D(8, kernel_size=(7, 7), padding = "SAME", strides = 1, activation='relu')(y)
    y0c = Model(inputs = input1, outputs = y0c)

    yc0 = keras.layers.concatenate([y0b.output, y0c.output])
    # =================================================
    y1b = Conv2D(16, kernel_size=(5, 5), padding = "SAME", strides = 1, activation='relu')(yc0)
    y1b = Model(inputs = input1, outputs = y1b)

    y1c = Conv2D(16, kernel_size=(7, 7), padding = "SAME", strides = 1, activation='relu')(yc0)
    ...
    # =================================================
    x0b = Conv2D(8, kernel_size=(5, 5), padding = "SAME", strides = 1, activation='relu')(x)
    x0b = Model(inputs = input2, outputs = x0b)

    x0c = Conv2D(8, kernel_size=(7, 7), padding = "SAME", strides = 1, activation='relu')(x)
    x0c = Model(inputs = input2, outputs = x0c)

    xc0 = keras.layers.concatenate([x0b.output, x0c.output])
    # =================================================
    x1b = Conv2D(16, kernel_size=(5, 5), padding = "SAME", strides = 1, activation='relu')(xc0)
    x1b = Model(inputs = input2, outputs = x1b)

    x1c = Conv2D(16, kernel_size=(7, 7), padding = "SAME", strides = 1, activation='relu')(xc0)
    ...
    # ============== DL ===============================
    json_path, hdf5_path = get_model_path("prediction_gnet4", ratio)

    delta, n_patience, batch_size, epoch_size = get_training_parameter("prediction_gnet4")
    # ============== YL ===============================
